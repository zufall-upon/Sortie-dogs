...
test("packed package exposes plugin and versioned runtime assets", async () => {
  ...
  try {
    ...
    assert.equal(installedPackage.version, "0.9.8");
    assert.equal(
    ...
    assert.equal(RUNTIME_ASSET_VERSION, "0.3.84-dispatch-recovery-v1");
    ...
    for (const asset of loaded.runtimeAssets) {
      assert.equal(asset.version, "0.3.84-dispatch-recovery-v1");
