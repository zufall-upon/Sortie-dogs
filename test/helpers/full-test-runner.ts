...
type ExecutionTransition = { status: ExecutionStatus; paths: string[]; at_ms: number };
...
async function runPhase(
  ...
  const args = ["--experimental-strip-types", "--import", "./test/setup.ts", "--test"];
  ...
  return await new Promise<ChildResult>((resolvePromise) => {
    ...
    const forward = (stream: NodeJS.ReadableStream, destination: NodeJS.WritableStream) => {
      ...
      stream.on("data", (chunk: Buffer) => {
        ...
        remainder = lines.pop() ?? "";
        for (const line of lines) destination.write(`SORTIE_FULL_BATCH_OUTPUT ${batch} ${line}\n`);
      });
      stream.on("end", () => {
        if (remainder.length > 0) destination.write(`SORTIE_FULL_BATCH_OUTPUT ${batch} ${remainder}\n`);
      });
    };
...
function recordExecution(record: ExecutionRecord, status: ExecutionStatus, paths: readonly string[], startedAt: number): void {
  record[status].push(...paths);
  record.transitions.push({ status, paths: [...paths], at_ms: Math.round(performance.now() - startedAt) });
}
...
async function runBatch(
  ...
  recordExecution(record, "started", paths, startedAt);
  const result = await runPhase(paths, concurrency, remaining(), batch, controller.signal);
  if (result.exit === 0 && result.stopped) recordExecution(record, "completed", paths, startedAt);
  if (result.exit !== 0 || !result.stopped) controller.abort();
  return result;
}
...
async function execute(): Promise<number> {
  ...
  recordExecution(record, "started", partition.processExclusive, startedAt);
  const processExclusive = await runPhase(partition.processExclusive, 1, remaining(), "process-exclusive");
  if (processExclusive.exit === 0 && processExclusive.stopped) recordExecution(record, "completed", partition.processExclusive, startedAt);
