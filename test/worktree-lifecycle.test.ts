...
test("worktree root replacement is fenced before Git can write outside", async (t) => {
  const value = await fixture("root-swap");
  try {
    const pin = await value.lifecycle.pinCleanBase();
    const original = `${value.worktrees}-original`;
    const outside = join(value.root, "outside");
    await mkdir(outside);
    await rename(value.worktrees, original);
    try {
      await symlink(outside, value.worktrees, process.platform === "win32" ? "junction" : "dir");
    } catch (error) {
      await rename(original, value.worktrees);
      t.skip(`filesystem link creation unavailable: ${(error as NodeJS.ErrnoException).code ?? "unknown"}`);
      return;
    }
    await errorCode(value.lifecycle.createMany({ pin, tasks: tasks(pin, ["escape-a", "escape-b"]) }), "invalid-request");
...
test("a predictable legacy junction cannot redirect nonce-target creation", async (t) => {
  const value = await fixture("legacy-junction");
  try {
    const pin = await value.lifecycle.pinCleanBase();
    const requested = tasks(pin, ["legacy-a", "legacy-b"]);
    const prefix = value.lifecycle.pathPrefixFor(requested[0]!.worktree);
    const legacyPath = prefix.slice(0, -1);
    const outside = join(value.root, "outside");
    await mkdir(outside);
    try {
      await symlink(outside, legacyPath, process.platform === "win32" ? "junction" : "dir");
    } catch (error) {
      t.skip(`filesystem link creation unavailable: ${(error as NodeJS.ErrnoException).code ?? "unknown"}`);
      return;
    }
    const created = await value.lifecycle.createMany({ pin, tasks: requested });
    assert.equal(created[0]!.path.startsWith(prefix), true);
    assert.notEqual(created[0]!.path, legacyPath);
    assert.deepEqual(await readdir(outside), []);
    await value.lifecycle.cleanup("legacy-a");
    await value.lifecycle.cleanup("legacy-b");
  } finally {
    await cleanupFixture(value);
  }
});

test("creation fails closed without external writes when a command hook precreates the discovered nonce target", async () => {
  const value = await fixture("target-precreate");
  try {
    const pin = await value.lifecycle.pinCleanBase();
    const outside = join(value.root, "outside");
    await mkdir(outside);
    ...
    privateLifecycle.git = async (args, cwd) => {
      if (occupied === undefined && args[0] === "worktree" && args[1] === "add") {
        occupied = args[4]!;
        await mkdir(occupied);
        await writeFile(join(occupied, "sentinel.txt"), "foreign\n");
      }
      return originalGit(args, cwd);
    };
    await errorCode(value.lifecycle.createMany({ pin, tasks: tasks(pin, ["occupied-a", "occupied-b"]) }), "git-failed");
    assert.equal(await readFile(join(occupied!, "sentinel.txt"), "utf8"), "foreign\n");
    assert.deepEqual(await readdir(outside), []);
    assert.deepEqual((await value.lifecycle.reconcile()).map(({ phase }) => phase), ["orphaned", "orphaned"]);
  } finally {
    await cleanupFixture(value);
...
test("cleanup rechecks target identity after unlock and never removes its replacement", async () => {
  const value = await fixture("cleanup-target-race");
  try {
    const pin = await value.lifecycle.pinCleanBase();
    const created = await value.lifecycle.createMany({ pin, tasks: tasks(pin, ["raced", "control"]) });
    const displaced = `${created[0]!.path}-displaced`;
    const sentinel = join(created[0]!.path, "sentinel.txt");
    const privateLifecycle = value.lifecycle as unknown as {
      git(args: readonly string[], cwd?: string): Promise<string>;
    };
    const originalGit = privateLifecycle.git.bind(value.lifecycle);
    privateLifecycle.git = async (args, cwd) => {
      const result = await originalGit(args, cwd);
      if (args[0] === "worktree" && args[1] === "unlock" && args[2] === created[0]!.path) {
        await rename(created[0]!.path, displaced);
        await mkdir(created[0]!.path);
        await writeFile(sentinel, "outside ownership\n");
      }
      return result;
    };
    await errorCode(value.lifecycle.cleanup("raced"), "unsafe-cleanup");
    assert.equal(await readFile(sentinel, "utf8"), "outside ownership\n");
...
test("cleanup detects an original-path swap before quarantine move and never invokes remove", async () => {
  const value = await fixture("cleanup-move-race");
  try {
    const pin = await value.lifecycle.pinCleanBase();
    const created = await value.lifecycle.createMany({ pin, tasks: tasks(pin, ["move-raced", "control"]) });
    const originalPath = created[0]!.path;
    const displaced = `${originalPath}-displaced`;
    const privateLifecycle = value.lifecycle as unknown as {
      git(args: readonly string[], cwd?: string): Promise<string>;
    };
    const originalGit = privateLifecycle.git.bind(value.lifecycle);
    ...
    let swapped = false;
    privateLifecycle.git = async (args, cwd) => {
      if (!swapped && args[0] === "worktree" && args[1] === "move") {
        swapped = true;
        await rename(originalPath, displaced);
        await mkdir(originalPath);
        await writeFile(join(originalPath, "sentinel.txt"), "foreign\n");
      }
      if (args[0] === "worktree" && args[1] === "remove") removeCalled = true;
      return originalGit(args, cwd);
    };
    await errorCode(value.lifecycle.cleanup("move-raced"), "unsafe-cleanup");
    assert.equal(removeCalled, false);
    assert.equal((await stat(displaced)).isDirectory(), true);
    const orphan = (await value.lifecycle.reconcile()).find(({ branch }) => branch === created[0]!.branch)!;
    assert.equal(orphan.phase, "orphaned");
    assert.equal(await readFile(join(orphan.path, "sentinel.txt"), "utf8"), "foreign\n");
  } finally {
    await cleanupFixture(value);
  }
});

test("cleanup removes only random quarantine when the old path is recreated after move", async () => {
  const value = await fixture("cleanup-old-path-race");
  try {
    const pin = await value.lifecycle.pinCleanBase();
    const created = await value.lifecycle.createMany({ pin, tasks: tasks(pin, ["old-raced", "control"]) });
    const oldPath = created[0]!.path;
    const sentinel = join(oldPath, "sentinel.txt");
    const privateLifecycle = value.lifecycle as unknown as {
      git(args: readonly string[], cwd?: string): Promise<string>;
    };
    const originalGit = privateLifecycle.git.bind(value.lifecycle);
    ...
    privateLifecycle.git = async (args, cwd) => {
      const result = await originalGit(args, cwd);
      if (quarantine === undefined && args[0] === "worktree" && args[1] === "move") {
        quarantine = args[3]!;
        await mkdir(oldPath);
    ...
    await value.lifecycle.cleanup("old-raced");
    assert.match(basename(quarantine!), /^rm-[0-9a-f]{16}-[0-9a-f]{32}$/u);
    assert.equal(await stat(quarantine!).catch(() => undefined), undefined);
...
test("inventory rejects zero and non-safe filesystem identities", async () => {
  const value = await fixture("zero-identity");
  try {
    const pin = await value.lifecycle.pinCleanBase();
    await value.lifecycle.createMany({ pin, tasks: tasks(pin, ["zero-a", "zero-b"]) });
    const inventory = JSON.parse(await readFile(inventoryPath(value), "utf8")) as {
      records: Array<{ targetDev: string; targetIno: string }>;
    };
    inventory.records[0]!.targetDev = "0";
    inventory.records[0]!.targetIno = "0";
    await writeFile(inventoryPath(value), JSON.stringify(inventory));
    await errorCode(WorktreeLifecycle.open({ repositoryRoot: value.repository }), "corrupt-inventory");
    inventory.records[0]!.targetDev = String(BigInt(Number.MAX_SAFE_INTEGER) + 1n);
    inventory.records[0]!.targetIno = "1";
    await writeFile(inventoryPath(value), JSON.stringify(inventory));
    await errorCode(WorktreeLifecycle.open({ repositoryRoot: value.repository }), "corrupt-inventory");
  } finally {
    await cleanupFixture(value);
