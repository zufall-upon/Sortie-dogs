...
export function createProfiledPlugin(profile: RuntimeProfile, assetVersion: string): OpenCodePlugin {
  return async (input, options) => {
    ...
    const proposalContract = "proposal_json must encode one JSON object with exactly these required fields and types: " +
      ...
      '{branch_create:{branch:string,start_ref:string},commit:{message:string},post_commit_validation:string[]}. By the existing canonical command identity, post_commit_validation must match declared goal validations and be the same-order contiguous suffix of final unit.validation, exclusive to that unit. git_lifecycle authorizes only host fixed-argv branch creation before worker spend and one explicit-path commit of the approved unit.write union immediately before that suffix; fresh existing-executor evidence remains required. It never authorizes arbitrary Git, overwrite, force, amend, push, add -A, or commit -a. No aliases or extra packet keys. ' +
    ...
    const planContract = "plan_json must encode the exact operator plan object: schema_version, acceptance, acceptance_proof, source_refs, " +
      ...
      '{branch_create:{branch:string,start_ref:string},commit:{message:string},post_commit_validation:string[]}. Its presence is root authorization for only a clean, non-overwriting ' +
