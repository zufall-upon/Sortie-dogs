...
export async function verifyGlobal(root, globalRoot, version) {
  ...
  if ((await readFile(join(globalRoot, 'sortie-dogs.version'), 'utf8')).trim() !== RUNTIME_ASSET_VERSION) throw Error('Global runtime marker mismatch');
  ...
  const execute = async (tool, args, options = {}) => {
    if (tool === 'global' || tool === 'global-verify') {
      if (tool === 'global') {
        ...
        const init = await execute('node', [join(args[1], 'node_modules/sortie-dogs/dist/cli/main.js'), 'init', '--global'],
          { ...options, env: { OPENCODE_CONFIG_DIR: args[1] } });
        if (init.code !== 0) return init;
      }
      return { code: 0, stdout: JSON.stringify(await verifyGlobal(root, args[1], version)), stderr: '' };
    }
    let executable = tool, argv = args;
    if (tool === 'npm') executable = manifest.npm;
    if (tool === 'gh') executable = manifest.gh;
    if (tool === 'cli') { executable = process.execPath; argv = [join(root, 'scripts/release-cli.mjs'), ...args]; }
...
# $otp = Read-Host 'npm OTP'; & $npm publish $tgz --access public --registry=$registry "--otp=$otp"; Remove-Variable otp
