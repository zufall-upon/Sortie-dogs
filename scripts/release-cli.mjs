...
async function command(executable, args, cwd, env, timeoutMs = 600_000) {
...
export async function inside(tgz, directory) {
  ...
  await mkdir(join(project, 'child/.sortie-dogs/contracts'), { recursive: true });
  ...
  const { RUNTIME_ASSET_VERSION } = await import(pathToFileURL(join(installed, 'dist/asset-version.js')).href);
  const { runtimeAssets } = await import(pathToFileURL(join(installed, 'dist/runtime-assets.js')).href);
  ...
  await writeFile(join(control, 'opencode.json'), JSON.stringify({ $schema: 'https://opencode.ai/config.json',
    plugin: [pathToFileURL(join(installed, 'dist/plugin/opencode.js')).href] }, null, 2));
  await command('node', [join(installed, 'dist/cli/main.js'), 'init', project], project, env);
  ...
  const handoff = join(project, 'child/.sortie-dogs/contracts/handoff.recovery.json');
  ...
  await writeFile(join(project, 'child/.sortie-dogs/contracts/recovery.operation-manifest.json'), JSON.stringify(manifest, null, 2));
  await writeFile(handoff, JSON.stringify({ version: '0.1.0', profile: 'minimal', id: 'recovery', created_at: new Date().toISOString(),
    ...
    ext: { 'sortie-dogs/write-gate': { project_root: join(project, 'child'), operation_manifest: '.sortie-dogs/contracts/recovery.operation-manifest.json' },
  ...
  async function cli(prompt, sessionID) {
    ...
    return jsonEvents(await command('timeout', ['--signal=TERM', '--kill-after=10s', '540s', 'opencode', 'run',
      '--format', 'json', '--print-logs', '--agent', 'dog-coordinator', ...(sessionID ? ['--session', sessionID] : []), prompt], project, env, 580_000));
  }
  ...
  const ledgerPath = join(project, '.git/sortie-dogs/run-flight', `${hash(sessionID)}.json`);
...
goal_expected_outcome: pass`;
  ...
  return { schema: 1, version: pkg.version, sha256: hash(await readFile(tgz)), runtimeMarker: RUNTIME_ASSET_VERSION,
...
async function main() {
  const [tgz, directory] = process.argv.slice(2).map(value => resolve(value));
  assert(tgz && directory, 'tarball and receipt directory required');
  if (process.platform !== 'win32') { console.log(JSON.stringify(await inside(tgz, directory))); return; }
  ...
  const output = await command('wsl.exe', ['-e', 'bash', '-ic', `timeout --signal=TERM --kill-after=10s 1440s node ${[script, linuxTgz, linuxDirectory].map(shellQuote).join(' ')}`], process.cwd(), {}, 1_500_000);
